...
def count_beta_matches(model_folder, c, subgraph_size_as_fraction, save=True):

    beta_path = os.path.join(model_folder, f'raw_betas_list_size_{subgraph_size_as_fraction}')
    ...
    if save==True:
        with open(os.path.join(model_folder,'distribution.txt'),'w') as f:
...
if __name__ == '__main__':
    ...
    arch_folder = [f for f in os.listdir(train_folder) if f[:3] != 'clf' and f[0]!= '.'][0]
    ...
    for size in [subgraph_size]:
        ...
        # print('compute_regression_results:',compute_regression_results)
        # print('beta path exists:',os.path.exists(beta_path))
        ...
        if args.get_match_distribution==True:
            #if all_raw_betas==None:
            #    try:
            #        all_raw_betas = pickle.load(open(beta_path, 'rb'))
            #    except:
            #        ret_str = f'Havent yet computed raw betas -- run the following: \npython collect_random_subgraphs.py --dataset_name {dataset_name} --compute_regression_results --numSubgraphs {args.numSubgraphs} --subgraph_size {args.subgraph_size} --seed {args.seed}'
            #        print(ret_str)
            ...
            # mu_natural, sigma_natural = count_beta_matches(dataset_name, args.numSubgraphs, args.subgraph_size, arch_path='default',  numLayers=numLayers, hDim=hDim, dropout=dropout, skip=skip, seed=args.seed,save=True)
            mu_natural, sigma_natural = count_beta_matches(model_folder, args.numSubgraphs, args.subgraph_size, save=True)